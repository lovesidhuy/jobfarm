...
def _safe_alert(msg: str, title: str = "") -> None:
    """Print warning to console instead of blocking GUI dialog."""
    prefix = f"[{title}] " if title else ""
    try:
        print(f"\n[WARNING] {prefix}{msg}\n", flush=True)
    except UnicodeEncodeError:
        print(f"\n[WARNING] {prefix}{msg.encode('ascii', 'replace').decode()}\n", flush=True)


def alert(msg: str, title: str = "") -> None:
    """Non-blocking alert: always prints to console. GUI popup only in interactive mode."""
    _safe_alert(msg, title)
    autonomous = os.environ.get("AUTONOMOUS_SUPERVISOR", "").strip().lower() in ("1", "true", "yes")
    if not autonomous:
        try:
            from pyautogui import alert as _gui_alert
            _gui_alert(msg, title)
        except Exception:
            pass
...
def resolve_project_path(path: str) -> str:
    """
    Resolve a config path relative to the repo root, not the process cwd.
    Supports segregating runtime data (data, logs, excels, resumes, backups)
    to a separate directory specified by JOBBOTS_DATA_DIR environment variable.
    """
    p_orig = os.path.expanduser(path)
    p = pathlib.Path(p_orig)
    if p.is_absolute():
        return str(p.resolve())

    # Check if this is a data/log/resume/excel path we want to segregate
    parts = p.parts
    if parts:
        first_part = parts[0]
        data_dir = os.environ.get("JOBBOTS_DATA_DIR", "").strip()
        if data_dir and first_part in ("data", "logs", "all excels", "all resumes", "backups"):
            resolved = pathlib.Path(data_dir) / p
...
def get_log_path():
    ...
    '''
    try:
        path = resolve_project_path(logs_folder_path + "/log.txt")
        return path.replace("//","/")
    except Exception as e:
        critical_error_log("Failed getting log path! So assigning default logs path: './logs/log.txt'", e)
        return resolve_project_path("logs/log.txt")
...
def print_lg(*msgs: str | dict, end: str = "\n", pretty: bool = False, flush: bool = False, from_critical: bool = False) -> None:
    ...
    for message in msgs:
        try:
            ...
            safe = str(message).encode("ascii", "replace").decode()
            pprint(safe) if pretty else print(safe, end=end, flush=flush)
        try:
            with open(__logs_file_path, 'a+', encoding="utf-8") as file:
                file.write(str(message) + end)
        except Exception as e:
            detail = f"[Logging] Could not write {__logs_file_path}: {type(e).__name__}: {e}"
            try:
